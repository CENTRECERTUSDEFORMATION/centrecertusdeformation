...
const EspaceParticipant = () => {
  ...
  const fetchUpcomingSeances = useCallback(async () => {
    if (!user?.id) return [];
    try {
      ...
      // Récupérer les formations pour les titres
      ...
      const formationsMap = {};
      formationsData?.forEach(f => { formationsMap[f.id] = f.title; });
      ...
      const seancesAvecInfos = seances.map(seance => {
        ...
        const formationTitle = formationsMap[inscription?.formation_id] || "Formation";
        return { 
          ...seance, 
          formation_id: inscription?.formation_id, 
          formation_titre: formationTitle,
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      const [formations, pending, seances] = await Promise.all([
        fetchMyFormations(),
        fetchPendingFormations(),
        fetchUpcomingSeances()
      ]);
      
      setMyFormations(formations);
      setPendingFormations(pending);
      setUpcomingSeances(seances);
      setDataFetched(true);
    } catch (err) {
  ...
  }, [user?.id, isApproved, fetchMyFormations, fetchPendingFormations, fetchUpcomingSeances]);
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      ...
      .on('postgres_changes', 
        ...
        () => {
          // Mettre à jour uniquement les séances
          fetchUpcomingSeances().then(setUpcomingSeances);
        }
      )
      .subscribe();
  ...
  }, [user, userType, isApproved, fetchUpcomingSeances]);
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 pt-20">
        <div className="max-w-7xl mx-auto p-6">
          ...
          <div className="bg-gradient-to-r from-[#1a56db] to-[#76c21f] text-white rounded-2xl p-8 mb-8">
            ...
            <p className="text-blue-100 mt-1">Bienvenue, {user?.user_metadata?.full_name || user?.email}</p>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {pendingFormations.map(ins => (
                  <div key={ins.id} className="bg-yellow-50 rounded-xl shadow-md overflow-hidden border border-yellow-200">
                    ...
                    <div className="p-5">
                      <p className="text-gray-600 text-sm mb-3 line-clamp-2">{ins.formations?.description}</p>
          ...
          <div className="mb-10">
            <h2 className="text-xl font-bold text-gray-800 mb-4 flex items-center gap-2">
              <span className="text-green-600">🎥</span> Vos réunions à venir
            </h2>
            ...
            {upcomingSeances.length === 0 ? (
              <div className="bg-white rounded-xl shadow-md p-8 text-center border border-gray-200">
                ...
                <p className="text-gray-500 text-sm mt-1">Les liens de réunion apparaîtront ici dès que votre formateur les partagera.</p>
              </div>
            ) : (
              <div className="space-y-4">
                {upcomingSeances.map(seance => (
                  <div key={seance.id} className="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow">
                    <div className="p-5">
                      <div className="flex justify-between items-start flex-wrap gap-3">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2 flex-wrap">
                            <span className="bg-blue-100 text-blue-700 text-xs px-2 py-1 rounded-full">{seance.formation_titre}</span>
                            <span className="bg-green-100 text-green-700 text-xs px-2 py-1 rounded-full">🟢 À venir</span>
                          </div>
                          <h3 className="text-lg font-bold text-gray-800">{seance.titre}</h3>
                          <div className="flex items-center gap-4 mt-2 text-sm text-gray-500 flex-wrap">
                            <span className="flex items-center gap-1">📅 {new Date(seance.date_seance).toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' })}</span>
                            <span className="flex items-center gap-1">⏰ {new Date(seance.date_seance).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}</span>
                          ...
                          {seance.lien_partage_le && new Date(seance.lien_partage_le) > new Date(Date.now() - 5 * 60 * 1000) && (
                            <div className="mt-2 p-2 bg-green-50 rounded-lg border border-green-200 inline-block">
                              <p className="text-xs text-green-700 flex items-center gap-1">🆕 Le lien vient d'être partagé par votre formateur !</p>
                            </div>
                          )}
                        </div>
                        <button 
                          onClick={() => rejoindreReunion(seance.lien_reunion)} 
                          className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-6 py-3 rounded-xl font-medium hover:shadow-lg transition-all flex items-center gap-2 whitespace-nowrap"
                          aria-label={`Rejoindre la réunion ${seance.titre}`}
                        >
                          <span className="text-lg">🎥</span>Rejoindre la réunion<span>→</span>
                        </button>
                      </div>
                    </div>
                  </div>
            ...
            {myFormations.length === 0 ? (
              <div className="bg-white rounded-xl shadow-md p-8 text-center border border-gray-200">
                ...
                <Link 
                  ...
                  aria-label="Découvrir les formations disponibles"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {myFormations.map(ins => (
                  <div key={ins.id} className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-200">
                    <div className="bg-gray-800 text-white p-4">
                      <h3 className="font-bold text-lg">{ins.formations?.title}</h3>
                      {ins.formations?.is_online && (
                        <span className="inline-block mt-1 text-xs bg-green-500 px-2 py-0.5 rounded-full">🌍 En ligne</span>
                      )}
                    </div>
                    <div className="p-5">
                      <p className="text-gray-600 text-sm line-clamp-3">{ins.formations?.description}</p>
                      <p className="text-xs text-gray-400 mt-3">Inscrit le {new Date(ins.created_at).toLocaleDateString()}</p>
                      {ins.formations?.is_online && (
